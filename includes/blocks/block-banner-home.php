<?php
$banner = get_field('group_banner_home') ?: [];

if (empty($banner['home_disable_section'])):

    $image_url        = $banner['home_image'] ?? '';
    $bg_overlay_color = $banner['home_overlay_color'] ?? '#000000a1';

    // Height logic
    $min_height = $banner['home_min_height_desktop'] ?? '';
    if (wp_is_mobile() && !empty($banner['home_min_height_mobile'])) {
        $min_height = $banner['home_min_height_mobile'];
    }
    if (is_numeric($min_height)) {
        $min_height .= 'px';
    }

    // Background position logic
    $bg_position = $banner['home_bg_position_desktop'] ?? '';
    if (wp_is_mobile() && !empty($banner['home_bg_position_mobile'])) {
        $bg_position = $banner['home_bg_position_mobile'];
    }

    // Build inline style
    $inline_style = '';

    if (!empty($min_height)) {
        $inline_style .= 'height:' . esc_attr($min_height) . ';';
    }

    if ($image_url) {
        $inline_style .= 'background-image:url(' . esc_url($image_url) . ');';

        if (!empty($bg_position)) {
            $inline_style .= 'background-position:' . esc_attr($bg_position) . ';';
        }
    }
?>
<section class="banner__section banner__section-home"
    <?php if (!empty($inline_style)): ?>
        style="<?php echo $inline_style; ?>"
    <?php endif; ?>
    aria-label="Banner">

    <div class="image-overlay" style="background-color: <?php echo esc_attr( $bg_overlay_color ); ?>;"></div>

    <div class="container">
        <div class="row">
            <div class="centerized-content">
                <?php
                if (!empty($banner['home_title'])) {
                    $title = $banner['home_title'];
                    $title_class = 'is-custom-title';
                } else {
                    $title = get_the_title();
                    $title_class = 'is-page-title';
                }
                ?>
                <?php if (!empty($title)): ?>
                    <h1 class="<?php echo esc_attr($title_class); ?>">
                        <?php echo esc_html($title); ?>
                    </h1>
                <?php endif; ?>

                <?php if (!empty($banner['home_subtitle'])): ?>
                    <p><?php echo $banner['home_subtitle']; ?></p>
                <?php endif; ?>

                <?php
                // Use only ACF Link field `home_button_link_1` (array with 'url' and 'title').
                $btn = $banner['home_button_link_1'] ?? null;
                if (!empty($btn) && is_array($btn) && !empty($btn['url']) && !empty($btn['title'])): ?>
                    <div class="default-btn">
                        <a href="<?php echo esc_url($btn['url']); ?>" class="link-btn" <?php echo !empty($btn['target']) ? 'target="' . esc_attr($btn['target']) . '" rel="noopener noreferrer"' : ''; ?>>
                            <?php echo esc_html($btn['title']); ?>
                        </a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    </div>

</section>
<?php endif; ?>
